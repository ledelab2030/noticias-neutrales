    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      {hero && (
        <article className="mb-8 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
            ...
            {getFuenteNombre(hero.fuente) && <span className="truncate">{getFuenteNombre(hero.fuente)}</span>}
          </div>
          <h2 className="mt-2 text-2xl font-semibold leading-snug text-gray-900 dark:text-white">
            <Link href={`/noticias/${hero.id}`} className="hover:text-blue-700 dark:hover:text-blue-300">{hero.titulo}</Link>
          </h2>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        {rest.map((n) => {
          ...
          return (
            <article key={n.id} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                <time>{fmt(n.fecha)}</time>
                {fuenteNombre && <span className="truncate">{fuenteNombre}</span>}
              </div>
              <h3 className="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                <Link href={`/noticias/${n.id}`} className="hover:text-blue-700 dark:hover:text-blue-300">{n.titulo}</Link>
              </h3>
      ...
      <section className="mt-12">
        ...
        <ul className="space-y-2">
          {recientes.map((n) => (
            <li key={n.id} className="flex items-center justify-between">
              <div>
                <Link href={`/noticias/${n.id}`} className="text-sm font-medium hover:underline">{n.titulo}</Link>
        ...
        <div className="mt-4">
          <Link href="/noticias" className="text-sm underline underline-offset-4">Más noticias e información</Link>
        </div>
      </section>
      ...
      <section className="mt-12">
        ...
        <div className="flex flex-wrap gap-2">
          {paisesDestacados.map((pais) => (
            <Link key={pais} href={`/pais/${encodeURIComponent(pais)}`} className="px-3 py-1 text-xs rounded-full border hover:bg-gray-50 dark:hover:bg-gray-800">
              {pais}
            </Link>
          ))}
        </div>
