...
type Nota = {
  ...
  fecha: string
  fuente: Fuente
  temas?: string[] // opcional: algunos items pueden no traerlo
}
...
export async function generateMetadata(
  ...
  const data = estiloDeVida as unknown as Nota[]
  const nota = data.find((n) => n.id === id)
...
export default async function NotaPage(
  ...
  const { id } = await params
  const data = estiloDeVida as unknown as ReadonlyArray<Nota>
  const nota = data.find((n) => n.id === id)
  if (!nota) return notFound()

  const chips = nota.temas && Array.isArray(nota.temas) ? nota.temas : []

  return (
    <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      <article className="max-w-none">
        <header className="mb-6">
          <p className="text-sm text-slate-500 dark:text-slate-400">{fechaBonita(nota.fecha)}</p>
