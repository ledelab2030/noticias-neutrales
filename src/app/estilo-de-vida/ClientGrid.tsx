...
function Card({ item }: { item: EstiloItem }) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 shadow-sm transition hover:shadow-md dark:border-slate-800">
      <div className="relative">
        <div className="aspect-[16/10] w-full bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900" />
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <h3 className="mt-2 line-clamp-2 text-lg font-semibold text-slate-900 dark:text-slate-50">
          <Link href={`/estilo-de-vida/${item.id}`} className="hover:text-[color:#0072CE] dark:hover:text-blue-300">
            {item.titulo}
          </Link>
        </h3>
        <p className="mt-2 line-clamp-3 text-slate-700 dark:text-slate-300">{item.resumen}</p>
        ...
        <div className="mt-4">
          <Link
            href={`/estilo-de-vida/${item.id}`}
            className="inline-flex items-center gap-2 text-[color:#0072CE] hover:underline dark:text-blue-300"
          >
            Leer más
